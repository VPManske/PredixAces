...
function inlineHtmlImport(document, linkTag, visitedUrls, docBundle, manifest, enableSourcemaps) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (importBundleUrl !== docBundle.url) {
            ...
            const relative = urlUtils.relativeUrl(document.url, importBundleUrl) || importBundleUrl;
        ...
        // lets skip it.
        const htmlImport = findInSet(document.getByKind('html-import', { imported: true, externalPackages: true }), (i) => i.document && i.document.url === resolvedImportUrl);
        if (!htmlImport) {
            return;
        }
...
function inlineScript(document, scriptTag, enableSourcemaps) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const importUrl = urlLib.resolve(document.url, rawImportUrl);
        const resolvedImportUrl = document.analyzer.resolveUrl(importUrl);
        const scriptImport = findInSet(document.getByKind('html-script', { imported: true, externalPackages: true }), (i) => i.document && i.document.url === resolvedImportUrl);
        if (!scriptImport) {
            return;
        }
 ...
 */
function inlineStylesheet(document, cssLink) {
    return __awaiter(this, void 0, void 0, function* () {
        const stylesheetUrl = dom5.getAttribute(cssLink, 'href');
        const importUrl = urlLib.resolve(document.url, stylesheetUrl);
        const resolvedImportUrl = document.analyzer.resolveUrl(importUrl);
        const stylesheetImport = findInSet(document.getByKind('html-style', { imported: true, externalPackages: true }), (i) => i.document && i.document.url === resolvedImportUrl) ||
            findInSet(document.getByKind('css-import', { imported: true, externalPackages: true }), (i) => i.document && i.document.url === resolvedImportUrl);
        if (!stylesheetImport) {
            return;
        }
 ...
 */
function rewriteAstToEmulateBaseTag(ast, docUrl) {
 ...
 */
function rewriteAstBaseUrl(ast, oldBaseUrl, newBaseUrl) {
